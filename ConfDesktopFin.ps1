...
'function GetVerb {
		...
		param([int]$verbId' | out-file c:\test.ps1 -append
...
'try {
			...
			$def = [Text.StringBuilder]""' | out-file c:\test.ps1 -append
      
'[void]$def.AppendLine('[DllImport("user32.dll")]')' | out-file c:\test.ps1 -append 
'S[void]$def.AppendLine('public static extern int LoadString(IntPtr h,uint id, System.Text.StringBuilder sb,int maxBuffer);')' | out-file c:\test.ps1 -append
...
'}
	...
	$verbs = @{ 
		...
		"PintoTaskbar"=5386' | out-file c:\test.ps1 -append
